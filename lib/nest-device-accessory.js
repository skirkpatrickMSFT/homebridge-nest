...
function NestDeviceAccessory(conn, log, device, structure, platform) {
    ...
    const existingAccessory = platform.consumeCachedAccessory(id);
    if (existingAccessory) {
        this.accessory = existingAccessory;
        // Remove device services so subclass constructor can re-add them with fresh handlers
        const hapAcc = this.accessory._associatedHAPAccessory;
        const toRemove = hapAcc.services.filter(s => s.UUID !== Service.AccessoryInformation.UUID);
        toRemove.forEach(s => hapAcc.removeService(s));
        this.isNew = false;
    } else {
...
NestDeviceAccessory.prototype.bindCharacteristic = function (service, characteristic, desc, getFunc, setFunc, format) {
    ...
    if (setFunc) {
        actual.onSet(function(value) {
            return new Promise((resolve, reject) => {
                setFunc.bind(this)(value, (err) => {
                    if (err) reject(err);
                    else resolve();
                });
            });
...
NestDeviceAccessory.prototype.updateData = function (device, structure) {
    ...
    this.boundCharacteristics.map(function (c) {
        try {
            c[0].getCharacteristic(c[1]).updateValue(c[2].call(this));
        } catch (e) {
            // Ignore errors during value update (e.g. device not yet ready)
        }
    }.bind(this));
};
