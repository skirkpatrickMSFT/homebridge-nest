...
class Connection {
        ...
        while (true) {
            ...
            if (this.config.googleAuth || this.config.refreshToken) {
                ...
                try {
                    ...
                    this.debug('Auth error detail: ' + (error.message || error.code));
                ...
                try {
                    const authenticatorUrl = new URL(this.config.authenticator);
                    if (authenticatorUrl.protocol !== 'https:') {
                        this.error('authenticator URL must use HTTPS. Aborting authentication.');
                        return false;
                    }
                    req = {
                        method: 'POST',
            ...
            try {
                ...
                try {
                    const transportUrl = new URL(body.urls.transport_url);
                    if (transportUrl.protocol !== 'https:') {
                        throw new Error('transport_url must use HTTPS');
                    }
                    this.transport_url = body.urls.transport_url;
                } catch(e) {
                    this.error('Invalid transport_url in auth response: ' + e.message);
                    return false;
                }
                ...
                if (error.status == 401 && error.response && error.response.data && error.response.data.truncated_phone_number) {
                    ...
                    if (this.config.pin) {
                        this.log('Using PIN from config.json.');
    ...
    updateProtobufData(resolve, handler) {
        ...
        return new Promise((res, rej) => {
            ...
            const cleanAndResolve = val => {
                ...
                clearInterval(cancelObserveTimer);
                clearInterval(pingTimer);
                timeoutTimer = null;
                cancelObserveTimer = null;
                pingTimer = null;
            ...
            timeoutTimer = setTimeout(timeoutFunction, API_OBSERVE_TIMEOUT_SECONDS * 1000);
            ...
            cancelObserveTimer = setInterval(() => {
                if (!this.token || !this.connected) {
            ...
            req.on('data', data => {
                ...
                protoChunks.push(data);
                protoChunksLength += data.length;

                if (pendingLength === 0) {
                    // Determine message length from accumulated header bytes
                    const headerBuffer = Buffer.concat(protoChunks);
                    pendingLength = varint.decode(headerBuffer, 1);
                    pendingLength += varint.decode.bytes + 1;
                    protoChunks = [headerBuffer];
                    protoChunksLength = headerBuffer.length;
                }

                if (protoChunksLength < pendingLength) {
                    return; // Accumulate more chunks before processing
                }

                // Materialize buffer only when we have a complete message
                protoBuffer = Buffer.concat(protoChunks);
                protoChunks = [];
                protoChunksLength = 0;

                // this.verbose('API observe POST: data received, length ' + data.length + ' (-> ' + protoBuffer.length + '/' + pendingLength + ')');
                while (protoBuffer.length >= pendingLength && pendingLength > 0) {
                ...
                // Carry any leftover bytes (partial next message) into chunks for next data event
                if (protoBuffer.length > 0) {
                    protoChunks = [protoBuffer];
                    protoChunksLength = protoBuffer.length;
    ...
    apiResponseToObjectTree(body) {
        ...
        for (const structureId in structures) {
            ...
            let whereLookup = {};
            if (body.where && body.where[structureId]) {
    ...
    mergeNestWithProtobufData(nestApiData, data) {
        ...
        for (const el in data) {
            if (!Object.prototype.hasOwnProperty.call(data, el) || el === '__proto__' || el === 'constructor' || el === 'prototype') continue;
    ...
    legacyToProtobufUpdate(body) {
        ...
        if (!output || output.length == 0) {
            return null;
        } else if (!this.TraitMap) {
            this.error('Protobuf TraitMap not yet initialized; skipping batch update');
            return null;
        } else {
...
function lookupTrait(proto, type_url) {
    if (traitCache.has(type_url)) {
        return traitCache.get(type_url);
    }
    let pbufTrait = null;
    ...
    traitCache.set(type_url, pbufTrait);
    return pbufTrait;
}
