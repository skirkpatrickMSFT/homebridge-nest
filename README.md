# homebridge-nest (Homebridge v2 Fork)

> **This is an unofficial fork of [chrisjshull/homebridge-nest](https://github.com/chrisjshull/homebridge-nest) updated for [Homebridge v2](https://github.com/homebridge/homebridge/wiki/Updating-To-Homebridge-v2.0) compatibility.**
>
> **Changes from upstream:**
> - Replaced deprecated `.on('get')`/`.on('set')` callback-style HAP characteristic handlers with the v2-required `.onGet()`/`.onSet()` promise-based API
> - Updated `updateData()` to push fresh values via `updateValue()` instead of the no-op `.value` trigger
> - Fixed error-state handlers when the Nest service is unreachable to use the new HAP-NodeJS v1 API

Nest plug-in for [Homebridge](https://github.com/homebridge/homebridge) using the native Nest API, based on [release 4.6.10](https://github.com/chrisjshull/homebridge-nest/releases/tag/v4.6.10).
...
- [Homebridge](https://github.com/homebridge/homebridge) v1.6.0 or later, **including Homebridge v2**

## Option 1: Install from this GitHub repository (recommended for Homebridge v2)
...
2. Clone this repository and install it as a local Homebridge plugin:
   ```bash
...
3. Update your Homebridge configuration file (`~/.homebridge/config.json`). See the **Configuration** section below.

4. Restart Homebridge.

## Option 2: Install the upstream release from npm (Homebridge v1 only)

> ⚠️ The upstream npm release does **not** work with Homebridge v2. Use Option 1 instead.
